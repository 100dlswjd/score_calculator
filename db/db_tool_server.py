...
try:
    cur.execute(f'''create table IF NOT EXISTS {db_name}.{table_name} (IDX int AUTO_INCREMENT  primary key, SITE int not null, REG_DATE DATETIME)''')
...
def get_total_score(site, site_name):
    # try:
        ...
        for name in names:
            sql += f"{name}, "
        sql = sql[:-2] + f" FROM {db_name}.{table_name} where SITE = {site}"
...
def get_1_1_score(site, site_name):
    try:
        ...
        for name in names:
            sql += f"{name}, "
        sql = sql[:-2] + f" FROM {db_name}.{table_name} where SITE = " + str(site)
...
def add_member(name):
    try:
        cur.execute(f"alter table {db_name}.{table_name} add COLUMN {name} int default null")
